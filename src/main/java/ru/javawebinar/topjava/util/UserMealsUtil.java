...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExceed> getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        /* Сгруппируем по дате */
        Map<LocalDate, Integer> mealsByDate = mealList.stream()
                .collect(Collectors.groupingBy(UserMeal::getDate, Collectors.summingInt(UserMeal::getCalories)));

        /* Отфильтруем, применим map, соберем результат */
        return mealList.stream()
                .filter(p -> TimeUtil.isBetween(p.getTime(), startTime, endTime))
                .map(meal -> createWithExceed(meal, new AtomicBoolean(mealsByDate.get(meal.getDate()) > caloriesPerDay)))
